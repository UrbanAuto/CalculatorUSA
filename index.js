...
function calc() {
  ...
  const includeInsurance = document.getElementById('insurance').checked;
  ...
  if (cityPrices !== undefined) {
    ...
    document.getElementById('material').textContent = `$${materialFee.toFixed(2)}`;

    // Розрахувати загальну ціну
    const totalPrice = cityPrices + service + kompleksValue + priceCar + strahuvanya + zbir + fixPrice + swift + mitoValue + pdvValue + acciseSum + hazardous + materialFee;
